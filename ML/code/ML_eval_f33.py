...
if __name__ == "__main__":
    ...
    probs = [20, 40, 60, 80, 100]

    for sanity in range(5):

        for exp in range(len(probs)):

            prob = probs[exp]

            print("Start attack with " + str(prob))
            attack = "17"
            models = []
            ...
            for i in range(5):
                if prob == 100:
                    models.append(dataPath + "_bad_30_35")
                else:
                    models.append(dataPath + "_bad_30_35_" + str(prob))

            softmax_test = softmax_model_test.SoftMaxModelTest(dataset, numClasses, numFeatures)
            weights = non_iid(models, numClasses, numParams, softmax_test, 100)
        ...
        np.savetxt("fig3results_amazon_" + str(sanity) + ".csv", results, delimiter=',')
