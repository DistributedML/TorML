...
def non_iid(model_names, numClasses, numParams, softmax_test, iter=3000):
    ...
    for i in xrange(iterations):
        ...
        deltas = np.reshape(total_delta, (numClients, numParams))


        cs = smp.cosine_similarity(ds) - np.eye(numClients)
        maxcs = np.max(cs, axis=1)
        for a in range(numClients):
            for b in range(numClients):
                if a == b:
                    continue
                if maxcs[a] < maxcs[b]:
                    cs[a][b] = cs[a][b] * maxcs[a]/maxcs[b]
        ...
        weights = weights + np.dot(deltas.T, wv)
